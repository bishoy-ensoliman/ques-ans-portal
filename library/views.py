...
class IndexView(generic.ListView):
    ...
    def get_queryset(self):
        ...
        if self.request.method == 'GET':
            ...
            if query:
                return models.DocFile.objects.filter(Q(title__contains=query))
        return models.DocFile.objects.all()


@login_required(login_url='/accounts/login/')
def add_document(request):
    if request.method == 'POST':
        form = forms.DocFileForm(request.POST, request.FILES)
        if form.is_valid():
        ...
        form = forms.DocFileForm()
    return render(request, 'library/add_document.html', {'form': form})
